'''
- multimodal_dataset_aligned.py
- MESA 수면 데이터에서 PPG/ECG 시계열을 불러와 train/val/test로 분할하고 모델 학습용 입력 텐서를 생성하는 데이터셋 모듈
- train/val/test용 h5 파일을 따로 만들지 않고, 원본 파일(PPG/ECG/index)만 사용
- Dataset 클래스가 index 파일과 subject ID 리스트를 이용해 런타임에 필요한 구간만 원본 h5에서 slice하여 읽어옴
- 물리적으로 파일을 나누지 않고도 DataLoader 단에서 train/val/test를 구분
'''
...
※ 주의
- SLEEPPPG_TEST_SUBJECTS에 포함된 모든 ID가 실제 데이터셋에 존재하는 것은 아님
- 데이터가 누락되었거나 윈도 수가 1200이 아닌 경우 해당 subject는 제외되어 test set에는 교집합만 포함됨 (코드에서 missing_subjects 경고 출력)
'''
...
def verify_test_set():
    data_paths = {
        'ppg': "./mesa-x/mesa_ppg_with_labels.h5",
        'real_ecg': "./mesa-x/mesa_real_ecg.h5",
        'index': "./mesa-x/mesa_subject_index.h5"
    }
